(() => {
  ...
  const scrubText = (value) => String(value)
    ...
    .replace(/thường dùng để dùng để/gi, "thường dùng để")
  ...
  const claimFromAnswer = (stem, answer, topic = "") => {
    const cleanStem = stripFinalPunctuation(scrubText(stem));
  ...
  const directQuestion = (chapter, fact, index) => ({
    ...
    topic: fact.topic,
    stem: capitalizeVisibleText(scrubText(fact.stem)),
  ...
  const listQuestion = (chapter, fact, facts, index) => {
    ...
    return {
      ...
      source: sourceLabel(chapter),
      topic: fact.topic,
      stem: scrubText(`Tổ hợp nào đúng và đủ nhất về ${topicLabel(fact.topic)}?`),
      choices: normalizeChoices([
  ...
  const countListQuestion = (chapter, fact, facts, index) => {
    ...
    return {
      ...
      source: sourceLabel(chapter),
      topic: fact.topic,
      stem: scrubText(`Về ${topicLabel(fact.topic)}, phương án nào vừa đúng số lượng vừa đúng nội dung?`),
      choices: normalizeChoices([
  ...
  const buildQuestions = (chapter) => {
    const variants = [
      ...
      (chapter, fact, facts, index) => correctionQuestion(chapter, fact, index),
      (chapter, fact, facts, index) => whyWrongQuestion(chapter, fact, index),
      (chapter, fact, facts, index) => listQuestion(chapter, fact, facts, index),
